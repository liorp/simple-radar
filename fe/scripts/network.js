import { API_BASE, HEALTH_CHECK_INTERVAL, RADAR_CHECK_INTERVAL, SERVER_TIMEOUT, MAX_DOTS } from './config.js';
import { cartesianToCanvas } from './radar.js';

// DOM elements
let statusElement;

// State
export let radarDots = [];
export let lastDataReceived = Date.now();

// Track history for trail effect
let trackHistory = new Map(); // track_id -> array of positions

// Initialize DOM references
export function initNetworkDOM() {
  statusElement = document.getElementById("status");
}

// Check if server is responding and clear dots if needed
export function checkServerAvailability() {
...
// Start radar polling
export function startRadarPolling() {
